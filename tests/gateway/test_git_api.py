...
async def test_list_repos_includes_non_git_directories(tmp_path):
    ...
    (ws / "plain").mkdir()
    (ws / "gitrepo").mkdir()
    subprocess.run(["git", "init", "-q"], cwd=ws / "gitrepo", check=True)
    # A repo with no commits has an unborn branch; its current_branch reports
    # empty. Commit once so a real branch name is exposed.
    (ws / "gitrepo" / "readme.md").write_text("hi")
    subprocess.run(["git", "-C", str(ws / "gitrepo"), "add", "."], check=True)
    subprocess.run(["git", "-C", str(ws / "gitrepo"), "-c", "user.email=t@t", "-c", "user.name=t",
