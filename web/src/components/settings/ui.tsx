...
export function PageTitle({ children }: { children: ReactNode }) {
  return (
    <h2 className="text-[22px] font-semibold text-codex-text tracking-tight m-0 mb-1.5">{children}</h2>
  );
}
...
export function SectionTitle({ children, className = "" }: { children: ReactNode; className?: string }) {
  return (
    <div className={`text-[13px] font-medium text-codex-text-secondary tracking-wide mb-2.5 ${className}`}>{children}</div>
  );
}

export function SettingsCard({ children, className = "" }: { children: ReactNode; className?: string }) {
  return (
    <div className={`bg-codex-surface border border-codex-border rounded-xl overflow-hidden mb-5 ${className}`}>
      {children}
    </div>
  );
}
  ...
  return (
    <div className="flex items-center justify-between gap-4 px-4 py-3.5 border-b border-codex-border last:border-b-0">
      <div className="flex-1 min-w-0">
        <div className="text-[13.5px] font-medium text-codex-text mb-0.5">{label}</div>
...
export function ActionBtn({
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className={`shrink-0 rounded-md px-3 py-1.5 text-[12.5px] border whitespace-nowrap disabled:opacity-45 disabled:cursor-not-allowed disabled:hover:bg-codex-active disabled:hover:text-codex-text-secondary ${
        danger
          ? "bg-[#3a1a1a] border-[#5a2a2a] text-[#f87171] hover:bg-[#4a2020]"
          : "bg-codex-elevated border-codex-border-strong text-codex-text-secondary hover:bg-codex-active hover:text-codex-text"
      }`}
    >
      {children}
    </button>
...
export function SegGroup({
  ...
  return (
    <div className="inline-flex bg-codex-elevated border border-codex-border-strong rounded-md overflow-hidden">
      {options.map((o) => (
        <button
          key={o.id}
          type="button"
          ...
          className={`px-3 py-1.5 text-[12.5px] whitespace-nowrap ${
            value === o.id ? "bg-codex-active text-codex-text" : "text-codex-muted hover:bg-codex-active hover:text-codex-text-secondary"
          }`}
        >
          {o.label}
        </button>
